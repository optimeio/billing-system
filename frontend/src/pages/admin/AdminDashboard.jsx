  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {statsCards.map((stat, i) => (
          <motion.div 
            ...
            className="glass p-6 rounded-2xl shadow-sm border border-slate-200 hover:shadow-md transition-shadow"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Recent Invoices */}
        <div className="lg:col-span-2 glass p-6 rounded-2xl border border-slate-200">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold text-slate-800">Recent Invoices</h2>
            <button className="text-primary text-sm font-bold hover:underline">View All</button>
          </div>
          <div className="overflow-x-auto overflow-y-hidden">
            <table className="w-full text-left" aria-label="Recent Invoices">
              ...
              <tbody className="divide-y divide-slate-100">
                {data?.recentInvoices.map((inv) => (
                  <tr key={inv._id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="py-4 font-bold text-slate-700">{inv.invoiceNumber}</td>
                    <td className="py-4 text-sm text-slate-600">{inv.customerName}</td>
        ...
        <div className="glass p-6 rounded-2xl border border-slate-200">
          <h2 className="text-xl font-bold text-slate-800 mb-6">Stock Alerts</h2>
          <div className="space-y-4">
            ...
            {data?.lowStockProducts.length === 0 && (
               <div className="flex flex-col items-center justify-center py-8 text-center">
                  <CheckCircle size={48} className="text-green-500 mb-3 opacity-20" />
                  <p className="text-slate-500 font-medium">All items well stocked</p>
               </div>
            )}
          </div>
        </div>
